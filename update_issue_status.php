...
// Required: issue_id and status
$issue_id = isset($input['issue_id']) ? intval($input['issue_id']) : 0;
$status = isset($input['status']) ? trim($input['status']) : '';
$comment = isset($input['comment']) ? trim($input['comment']) : null;
$assigned_technician_id = isset($input['assigned_technician_id']) && is_numeric($input['assigned_technician_id']) ? intval($input['assigned_technician_id']) : null;

if ($issue_id <= 0) { echo json_encode(["status"=>"error","message"=>"issue_id is required"]); exit; }
if ($status === '') { echo json_encode(["status"=>"error","message"=>"status is required"]); exit; }
